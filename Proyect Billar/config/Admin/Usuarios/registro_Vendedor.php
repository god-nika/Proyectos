...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $password = $_POST['password']; // No usar hash en pruebas, pero deberías agregarlo en producción
    $rol = $_POST['rol']; // Rol seleccionado (2 para Vendedor, 3 para Administrador)
    ...
    if ($result_check->num_rows > 0) {
        ...
        window.location.href = '../../../Formularios/Admin/Usuarios/RegisVendedor.php'; // Redirigir al formulario
        </script>";
    } else {
        ...
        try {
            ...
            // Insertar los datos en la tabla 'user_rol' con el rol seleccionado
            $sql_user_rol = "INSERT INTO user_rol (id_Persona, id_Rol, Contraseña) 
                             VALUES (?, ?, ?)";
            $stmt_user_rol = $conn->prepare($sql_user_rol);
            $stmt_user_rol->bind_param("iis", $id_Persona, $rol, $password); // $rol es el rol seleccionado (Vendedor o Admin)
            ...
            window.location.href = '../../../Formularios/Admin/Usuarios/Usuariodate.php'; // Redirigir al formulario
            </script>";
